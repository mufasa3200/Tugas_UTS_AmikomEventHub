    ...
    <section id="events" class="max-w-7xl mx-auto px-6 py-20">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
            ...
            <div class="flex flex-wrap gap-3">
                <a href="{{ url('/') }}" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-xl text-black transition font-bold text-sm">
                    Semua Kategori
                </a>

                @foreach($categories as $cat)
                    <a href="{{ url('/?category=' . $cat->slug) }}" 
                    class="px-4 py-2 bg-indigo-100 hover:bg-indigo-200 text-indigo-700 rounded-xl shadow-sm transition font-bold text-sm">
                        {{ $cat->name }}
                    </a>
                @endforeach
            </div>
